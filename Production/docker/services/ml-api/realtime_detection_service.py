...
# Configuration des alertes via variables d'environnement
ALERT_CONFIG = {
    "enable_logging": os.getenv('ALERT_ENABLE_LOGGING', 'true').lower() == 'true',
    "enable_webhooks": os.getenv('ALERT_ENABLE_WEBHOOKS', 'false').lower() == 'true',
    "webhook_url": os.getenv('ALERT_WEBHOOK_URL', None),
    "log_file": os.getenv('ALERT_LOG_FILE', 'alerts.log')
}

from DEV.config import FEATURE_NAMES
...
def send_webhook_alert(alert_data: Dict):
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info("Webhook alert envoyé avec succès")
        else:
            logger.error(f"Erreur lors de l'envoi du webhook: {response.status_code} - {response.text}")
